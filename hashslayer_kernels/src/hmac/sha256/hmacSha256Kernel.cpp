...
static void test_hmac_sha256(hls::stream<ap_uint<32> >& keyStrm,
                           hls::stream<ap_uint<32> >& msgStrm,
                           hls::stream<ap_uint<64> >& lenStrm,
                           hls::stream<bool>& eLenStrm,
                           hls::stream<ap_uint<256> >& hshStrm,
                           hls::stream<bool>& eHshStrm) {
    xf::security::hmac<32, 64, 256, 32, 64, sha256_wrapper>(keyStrm, msgStrm, lenStrm, eLenStrm, hshStrm, eHshStrm);
}
...
template <unsigned int _channelNumber>
static void hmacSha256Parallel(hls::stream<ap_uint<32> > keyStrm[_channelNumber],
                             hls::stream<ap_uint<32> > msgStrm[_channelNumber],
                             hls::stream<ap_uint<64> > msgLenStrm[_channelNumber],
                             hls::stream<bool> eMsgLenStrm[_channelNumber],
...
#pragma HLS unroll
        test_hmac_sha256(keyStrm[i], msgStrm[i], msgLenStrm[i], eMsgLenStrm[i], hshStrm[i], eHshStrm[i]);
    }
}
...
// @brief top of kernel
extern "C" void hmacSha256Kernel_1(ap_uint<512> inputData[(1 << 20) + 100], ap_uint<512> outputData[1 << 20]) {
#pragma HLS dataflow
...
#pragma HLS resource variable = burstLenStrm core = FIFO_LUTRAM
    ...
    hmacSha256Parallel<_channelNumber>(keyStrm, msgStrm, msgLenStrm, eMsgLenStrm, hshStrm, eHshStrm);
