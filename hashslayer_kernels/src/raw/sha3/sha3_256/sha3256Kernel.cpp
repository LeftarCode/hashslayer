...
static void test_sha3_256(hls::stream<ap_uint<64> >& msgStrm,
                           hls::stream<ap_uint<128> >& lenStrm,
                           hls::stream<bool>& eLenStrm,
                           hls::stream<ap_uint<256> >& hshStrm,
                           hls::stream<bool>& eHshStrm) {
    xf::security::sha3_256(msgStrm, lenStrm, eLenStrm, hshStrm, eHshStrm);
}
...
template <unsigned int _channelNumber>
static void sha3256Parallel(hls::stream<ap_uint<64> > msgStrm[_channelNumber],
                             hls::stream<ap_uint<128> > msgLenStrm[_channelNumber],
                             hls::stream<bool> eMsgLenStrm[_channelNumber],
...
#pragma HLS unroll
    	test_sha3_256(msgStrm[i], msgLenStrm[i], eMsgLenStrm[i], hshStrm[i], eHshStrm[i]);
    }
}
...
// @brief top of kernel
extern "C" void sha3256Kernel(ap_uint<512> inputData[(1 << 20) + 100], ap_uint<512> outputData[1 << 20]) {
#pragma HLS dataflow
...
#pragma HLS resource variable = burstLenStrm core = FIFO_LUTRAM
    ...
    sha3256Parallel<_channelNumber>(msgStrm, msgLenStrm, eMsgLenStrm, hshStrm, eHshStrm);
